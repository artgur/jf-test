                ...
                script: [
                    ...
                    script: '''
                        ...
                        def versionComparator(versions) {
                          versions.sort {a, b ->
                            def a1 = a.tokenize('.'), b1 = b.tokenize('.')
                            for (i in 0..<[a1.size(), b1.size()].min())
                              if (a1[i].replace("master", "0").toInteger() != b1[i].replace("master","0").toInteger() ) return a1[i] <=> b1[i]
                            0
                          }
                        }
                        // получаем список веток
                        def branches = versionComparator(getBranchNames("Builds/HCS_RELEASE").name).reverse()
